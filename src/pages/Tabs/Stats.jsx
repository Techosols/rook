...
function Stats() {
  ...
  const fetchStats = async (zipCode) => {
    ...
    try {
      const [exactRes, nearbyRes] = await Promise.all([
        ...
        fetch(
          `/api/fetch-data?endpoint=stats/zip/${zipCode}/scope/withinradius`
        ),
      ]);
      const [exactResData, nearbyResData] = await Promise.all([
        exactRes.ok ? exactRes.json() : null,
  ...
  return (
    <section
      className={`bg-background dark:bg-background-dark text-text dark:text-text-dark min-h-[60vh] py-6`}
    >
        ...
        {statsFound === null && !statData && !nearbyStatsData && (
          <div className="flex flex-col justify-center items-center min-h-[40vh] py-8">
            <BarChart2
              ...
              strokeWidth={1.5}
            />
            <p className="text-xl font-semibold text-gray-700 dark:text-gray-50 mb-2">
              Discover Rook Community Stats
            </p>
            <p className="text-gray-500 text-center max-w-md">
              Curious about the diversity, backgrounds, and interests of Rook
              members in your area? Enter a US-based zip code above to explore
              real stats and get inspired to connect!
            </p>
          </div>
        )}
        {statsFound === false && (
          <div className="flex flex-col justify-center items-center min-h-[40vh] py-8">
            ...
            <p className="text-gray-400">
              We could not find any stats for{" "}
              <span className="font-bold text-primary">{searchedZip}</span>
            </p>
          </div>
        )}
            ...
            {statData ? (
              <div className="p-0 md:p-2">
                <div className="text-center">
                  <h3 className="text-2xl sm:text-xl font-bold mb-1">
                    Stats for {searchedZip}
                  </h3>
                  <p className="text-gray-400 mb-2">
                    Here are some stats about Rook members in your area
                  </p>
                </div>
                <div className="block space-y-4">
                  {statData &&
                    Object.entries(statData).map(
                      ([category, values]) =>
                        Object.keys(values).length > 0 && (
                          <div
                            key={category}
                            className="bg-white dark:bg-background-dark rounded-xl hover:shadow-md p-3 sm:p-4"
                          >
                            <h4 className="font-semibold text-base sm:text-lg capitalize text-start mb-2">
                              By{" "}
                              {category
                                .replace(/([A-Z])/g, " $1")
              ...
              <div className="flex justify-center items-center">
                <p className="w-full">
                  We could not find any members in your exact ZIP code. See
                  right panel for stats around your ZIP code.
                </p>
              </div>
            )}
            {nearbyStatsData ? (
              <div className="p-0 md:p-2">
                <div className="text-center">
                  <h3 className="text-2xl sm:text-xl font-bold mb-1">
                    Stats for 25 miles around {searchedZip}
                  </h3>
                  <p className="text-gray-400 mb-2">
                    Here are some stats about Rook members around your area
                  </p>
                </div>
                <div className="block space-y-4">
                  {Object.entries(nearbyStatsData).map(
                    ([category, values]) =>
                      Object.keys(values).length > 0 && (
                        <div
                          key={category}
