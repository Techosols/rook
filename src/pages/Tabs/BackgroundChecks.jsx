import React from "react";
import useTheme from "../../hooks/useTheme";

function BackgroundChecks() {
  const { theme } = useTheme();

  return (
    <section className="px-2 py-8  bg-background dark:bg-background-dark text-text dark:text-text-dark">
      <div className="flex justify-between items-center flex-col lg:flex-row gap-8 max-w-[1350px] mx-auto">
        <div>
          <h1 className="text-xl md:text-2xl lg:text-3xl font-normal font-nunito text-left">
            Safe connections with real people
          </h1>
          <ul className="w-full max-w-2xl text-left">
            <div className="flex gap-2">
              <p className="mt-2">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 256 256"
                >
                  <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z " fill={theme === 'dark' ? 'white' : 'dark'}></path>
                </svg>
              </p>
              <li className="py-2">
                All Rook users consent to a background check when signing up,
                and also consent to a summary of the results being shown on
                their profile.
              </li>
            </div>
            <div className="flex gap-2">
              <p className="mt-2">
                 <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 256 256"
                >
                  <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z " fill={theme === 'dark' ? 'white' : 'dark'}></path>
                </svg>
              </p>
              <li className="py-2">
                All Rook users consent to a background check when signing up,
                and also consent to a summary of the results being shown on
                their profile.
              </li>
            </div>
            <div className="flex gap-2">
              <p className="mt-2">
                 <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 256 256"
                >
                  <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z " fill={theme === 'dark' ? 'white' : 'dark'}></path>
                </svg>
              </p>
              <li className="py-2">
                Background checks are performed by a third-party company.
              </li>
            </div>
            <div className="flex gap-2">
              <p className="mt-2">
                 <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 256 256"
                >
                  <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z " fill={theme === 'dark' ? 'white' : 'dark'}></path>
                </svg>
              </p>
              <li className="py-2">
                You can start using Rook while your check is in progress, but
                some features may only be available after your background check
                finishes.
              </li>
            </div>
            <a href="" className="text-primary dark:text-primary-dark">
              <li className="py-2 text-center">
                Full sign-up process and what happens after
              </li>
            </a>
          </ul>
        </div>
        <div>
          <img
            src="/Images/couple-altar.jpg"
            alt=""
            className="rounded-2xl w-full h-auto"
          />
        </div>
      </div>
      <div className="flex justify-between items-center flex-col lg:flex-row gap-8 max-w-[1350px] mx-auto mt-12">
        <div>
          <h1 className="text-xl md:text-2xl lg:text-3xl font-normal font-poppins text-left">
            Know real things about people
          </h1>
          <ul className="w-full max-w-2xl text-left">
            <div className="flex gap-2">
              <p className="mt-2">
                 <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 256 256"
                >
                  <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z " fill={theme === 'dark' ? 'white' : 'dark'}></path>
                </svg>
              </p>
              <li className="py-2">
                All Rook users consent to a background check when signing up,
                and also consent to a summary of the results being shown on
                their profile.
              </li>
            </div>
            <div className="flex gap-2">
              <p className="mt-2">
                 <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 256 256"
                >
                  <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z " fill={theme === 'dark' ? 'white' : 'dark'}></path>
                </svg>
              </p>
              <li className="py-2">
                All Rook users consent to a background check when signing up,
                and also consent to a summary of the results being shown on
                their profile.
              </li>
            </div>
            <div className="flex gap-2">
              <p className="mt-2">
                 <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 256 256"
                >
                  <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z " fill={theme === 'dark' ? 'white' : 'dark'}></path>
                </svg>
              </p>
              <li className="py-2">
                Background checks are performed by a third-party company.
              </li>
            </div>
            <div className="flex gap-2">
              <p className="mt-2">
                 <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 256 256"
                >
                  <path d="M229.66,77.66l-128,128a8,8,0,0,1-11.32,0l-56-56a8,8,0,0,1,11.32-11.32L96,188.69,218.34,66.34a8,8,0,0,1,11.32,11.32Z " fill={theme === 'dark' ? 'white' : 'dark'}></path>
                </svg>
              </p>
              <li className="py-2">
                You can start using Rook while your check is in progress, but
                some features may only be available after your background check
                finishes.
              </li>
            </div>
          </ul>
        </div>
        <div>
          <img
            src="/Images/couple-altar.jpg"
            alt=""
            className="rounded-2xl w-full h-auto"
          />
        </div>
      </div>
    </section>
  );
}

export default BackgroundChecks;
