...
function Select({ value, onChange, options = [], className = '', placeholder = 'Select an option', ...props }) {
    ...
    useEffect(() => {
        if (selectRef.current) {
            selectRef.current.classList.remove('animate-pulse');
            void selectRef.current.offsetWidth;
            selectRef.current.classList.add('animate-pulse');
            setTimeout(() => {
                selectRef.current && selectRef.current.classList.remove('animate-pulse');
            }, 300);
        }
    }, [value]);
    ...
    return (
        <div className={`relative w-full sm:w-80 md:w-96 group ${className}`}>
            ...
            >
                ...
                {options.map((opt, idx) =>
                    typeof opt === 'object' ? (
                        <option key={opt.value ?? idx} value={opt.value} className="font-semibold">
                            {opt.label}
                        </option>
                    ) : (
                        <option key={opt} value={opt} className="font-semibold">{opt}</option>
                    )
                )}
            </select>
