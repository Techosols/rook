...
function MatchesContainer({ title = 'Matches', data, endPoint = 'GetYourMatches', isLoadingMatches = false }) {
    ...
    const hasResults = Array.isArray(uiData?.results) && uiData.results.length > 0;
    ...
    const noDataMap = (() => {
        ...
        if (key.includes('match')) return {
            ...
            description: 'We haven\'t found any matches for your preferences. Try broadening your filters to see more people.',
            primary: { label: 'Adjust filters', action: () => {/* todo: open filters */} }
        };
        if (key.includes('connection')) return {
            icon: <Users className="w-12 h-12 text-indigo-500" />,
            headline: 'No connections',
            description: 'You don\'t have any connections yet. Send someone a connection request to get started.',
            primary: { label: 'Browse profiles', action: () => fetchPage(1) }
        };
        if (key.includes('bookmarked')) return {
            icon: <Bookmark className="w-12 h-12 text-amber-500" />,
            headline: 'No bookmarks',
            description: 'You haven\'t bookmarked any profiles. Bookmark interesting people to find them later.',
            primary: { label: 'Explore profiles', action: () => fetchPage(1) }
        };
        if (key.includes('profile') || key.includes('profiles')) return {
            icon: <User className="w-12 h-12 text-green-500" />,
            headline: 'No profiles found',
            description: 'There are no profiles matching this view. Try removing filters or searching for a different area.',
        ...
        if (key.includes('blocked')) return {
            ...
            description: 'You haven\'t blocked anyone yet. Block profiles from a profile card if needed.',
            primary: { label: 'Browse profiles', action: () => fetchPage(1) }
        };
        if (key.includes('ignored')) return {
            icon: <BellOff className="w-12 h-12 text-gray-500" />,
            headline: 'No ignored profiles',
            description: 'You haven\'t ignored any profiles. Use ignore to remove profiles from suggestions.',
            primary: { label: 'Explore suggestions', action: () => fetchPage(1) }
        };
        if (key.includes('random')) return {
            icon: <Shuffle className="w-12 h-12 text-purple-500" />,
            headline: 'No random picks',
            description: 'There are no random profiles right now. Try again later or broaden your search.',
        ...
        return {
            ...
            headline: 'No results',
            description: 'We couldn\'t find anything for this view. Try adjusting filters or come back later.',
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        ...
                        <div className="flex items-center space-x-3">
                            <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-3">
                                {(() => {
                                    const key = (title || '').toString().toLowerCase();
                                    if (key.includes('match')) return <Heart className="w-7 h-7 text-white" />;
                                    if (key.includes('connection') && key.includes('sent')) return <Send className="w-7 h-7 text-white" />;
                                    if (key.includes('connection') && key.includes('received')) return <Inbox className="w-7 h-7 text-white" />;
                                    if (key === 'connections' || key.includes('connection') ) return <Users className="w-7 h-7 text-white" />;
                                    if (key.includes('bookmarked') && key.includes('accept')) return <Star className="w-7 h-7 text-white" />;
                                    if (key.includes('bookmarked')) return <Bookmark className="w-7 h-7 text-white" />;
                                    if (key.includes('profile') && key.includes('view')) return <Eye className="w-7 h-7 text-white" />;
                                    if (key.includes('profiles')) return <User className="w-7 h-7 text-white" />;
                                    if (key.includes('blocked')) return <UserMinus className="w-7 h-7 text-white" />;
                                    if (key.includes('ignored')) return <BellOff className="w-7 h-7 text-white" />;
                                    if (key.includes('random')) return <Shuffle className="w-7 h-7 text-white" />;
                ...
                {isLoadingMatches ? (
                    <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                            {[1,2,3].map(i => (
                                <div key={i} className="p-4 border border-gray-100 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 shadow-sm animate-pulse">
                    ...
                    <>
                        <div className="space-y-4">
                            {filteredData.map((match) => (
                                <ProfileCard key={match.profileId} profile={match} showEvents={title === 'Your Matches'} />
                            ))}
                        </div>
                    </>
                ) : (
                    <div className="py-16 flex flex-col items-center justify-center text-center space-y-6 h-full">
                        ...
                        <div className="max-w-xl">
                            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{noDataMap.headline}</h3>
                            <p className="text-sm text-gray-600 dark:text-gray-400">{noDataMap.description}</p>
                        </div>
                    </div>
                )}
            </div>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <button
                                ...
                                disabled={currentPage === totalPages || fetching || isLoadingMatches}
                            ...
                            >
                                <span className="text-sm font-medium">{fetching || isLoadingMatches ? 'Loading...' : 'Next'}</span>
