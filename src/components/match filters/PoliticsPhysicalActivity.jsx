...
function PoliticsPhysicalActivity() {
  ...
  const { excludePoliticalAffiliations, excludePhysicalActivityIndexes } = useFilter();
      ...
      <FormSection title="Political Affiliation">
        ...
        <div className="border border-gray-300 dark:border-gray-600 rounded-lg p-3 max-h-60 overflow-y-auto flex flex-col gap-2">
          {excludePoliticalAffiliations?.map((affiliation) => (
            <Checkbox key={affiliation?.key} label={affiliation?.name} checked={excludePoliticalAffiliations?.excluded} />
          ))}
        </div>
      </FormSection>
      <FormSection title="Physical Activity Index">
        ...
        <div className="border border-gray-300 dark:border-gray-600 rounded-lg p-3 max-h-60 overflow-y-auto flex flex-col gap-2">
          {excludePhysicalActivityIndexes?.map((level) => (
            <Checkbox key={level?.key} label={level?.name} checked={excludePhysicalActivityIndexes?.excluded} />
          ))}
        </div>
      </FormSection>
    </div>
